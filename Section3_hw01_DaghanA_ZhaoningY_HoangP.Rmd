...
```{r}
logit.mod.full <- glm(formula = sanders_preference ~ age + race_white + party + gender, 
               family = binomial(link = logit), data = dt )
```
...
df.L <- dt[dt$race_white==race_white.L & dt$party==party.L & dt$gender==gender.L,]  
df.R <- dt[dt$race_white==race_white.R & dt$party==party.R & dt$gender==gender.R,]
```

```{r}
# Creating aggregated data for LEFT PLOT ###########################################
w.L <- aggregate(formula = as.numeric(df.L$sanders_preference)-1 ~ df.L$age,  FUN = sum)     # sanders supporters at each age
n.L <- aggregate(formula = as.numeric(df.L$sanders_preference) ~ df.L$age,  FUN = length)  # total voters at each age

names(w.L)[1] <- "age"
names(w.L)[2] <- "preference"
names(n.L)[1] <- "age"
names(n.L)[2] <- "preference"

w.n.L <- data.frame(age = w.L$age, n=n.L$preference, w=w.L$preference, 
                  ratio = round(w.L$preference/n.L$preference,4)) 

# Creating aggregated data for LEFT PLOT ###########################################
w.R <- aggregate(formula = as.numeric(df.R$sanders_preference)-1 ~ df.R$age,  FUN = sum)     # sanders supporters at each age
n.R <- aggregate(formula = as.numeric(df.R$sanders_preference) ~ df.R$age,  FUN = length)  # total voters at each age

names(w.R)[1] <- "age"
names(w.R)[2] <- "preference"
names(n.R)[1] <- "age"
names(n.R)[2] <- "preference"

w.n.R <- data.frame(age = w.R$age, n=n.R$preference, w=w.R$preference, 
                  ratio = round(w.R$preference/n.R$preference,4)) 
...
# Plot data points
symbols(x = w.L$age, y = w.L$preference/n.L$preference, circles = sqrt(n.L$preference), inches = 0.1,
        xlab = "age", ylab = "Estimated probability", xlim = c(15,100),
        panel.first = grid(col = "gray", lty = "dotted"),
        main = paste("race:", race_white.L," party:", party.L, ", gender:", gender.L))

# Plot model fit
curve(expr = predict(object = logit.mod.full, 
                     newdata = data.frame(age = x, race_white = race_white.L, party=party.L, gender=gender.L), 
                     type = "response"),
      col = "red", add = TRUE, xlim = c(15,100), ylim=c(0,1),
      xlab = "age", ylab = expression(hat(pi)))

# Plot C.I. bands
curve(expr = wald.ci.pi(newdata = data.frame(age = x, race_white = race_white.L, party=party.L, gender=gender.L), 
...
# Legend
# legend(x = 20, y = 0.2, legend = c("Full model", "95% individual C.I."),
#                                lty = c("solid", "dotdash"), col = c("red", "blue"), bty = "n")

legend("topright", legend = c("Full model", "95% individual C.I."),
                               lty = c("solid", "dotdash"), col = c("red", "blue"), bty = "n")

# RIGHT SIDE PLOT ###############################################################################

# Plot data points
symbols(x = w.R$age, y = w.R$preference/n.R$preference, circles = sqrt(n.R$preference), inches = 0.1,
        xlab = "age", ylab = "Estimated probability", xlim = c(15,100),
        panel.first = grid(col = "gray", lty = "dotted"),
        main = paste("race:", race_white.R," party:", party.R, ", gender:", gender.R))

# Plot model fit
curve(expr = predict(object = logit.mod.full, 
                     newdata = data.frame(age = x, race_white = race_white.R, party=party.R, gender=gender.R), type = "response"),
      col = "red", add = TRUE, xlim = c(15,100), ylim=c(0,1),
      xlab = "age", ylab =expression(hat(pi)))

# Plot C.I. bands
curve(expr = wald.ci.pi(newdata = data.frame(age = x, race_white =race_white.R, party=party.R, gender=gender.R), 
                        mod.fit.obj = logit.mod.full, alpha = 0.05)$lower,
      col = "blue", lty= "dotdash", add = TRUE, xlim = c(15,100))
