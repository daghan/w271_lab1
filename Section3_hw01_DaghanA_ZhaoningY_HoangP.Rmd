...
Our intution to include party affiliation as an explanatory variable is backed by the Chi-Square independence test. Witht the p-value close to 0, we can accept the alternative hypothesis that party and preference_bernie are dependent and therefore **party variable should be added to the model.**
...
```{r}
t <- table(dt[,.(Gender = ifelse(gender=="F","Female","Male"), Preference = ifelse(sanders_preference==1, "Sanders", "Hillary"))])
t

round(t/sum(t),2)
chisq.test(t)
```
Both males and females are more likely to prefer Sanders. But the ratio is nearly identical: $P(prefers\_sanders | female ) = \frac{0.27}{0.47} = 0.58$ and $P(prefers\_sanders | male ) = \frac{0.30}{0.52} = 0.58$. Our intuition is further backed by the very high p-value for indepence test. We can't reject the null hypothesis that the gender and preference_sanders are independent. **So we will not include gender in our model.**
...
The estimated odds ratio for race_white is $$\hat{OR}_{race\_white} = exp(\hat{\beta}_4)$$
