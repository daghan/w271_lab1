...
```
We observe that 8 out of 9 NAs are female and this could be problematic. However, given that gender is not a meaningful explonatory variable for preference_bernie and the relatively low number of data points with missing values (9 out of 1200), and that all other variables deviate very little between the NAs subset and the rest of the data, we can justify our decision to remove NA subset from the original dataset. 
...
Our intution to include party affiliation as an explanatory variable is backed by the Chi-Square independence test. Witht the p-value close to ), **we can accept the alternative hypothesis that party and preference_bernie** are dependent. 
...
chisq.test(t)
```
Both males and females are more likely to prefer Sanders. $P(prefers\_sanders | female ) = \frac{0.27}{0.47} = 0.58$ and $P(prefers\_sanders | male ) = \frac{0.30}{0.52} = 0.58$. Our intuition is further back by the very high p-value for indepence test. We can't reject the null hypothesis that the gender and preference_sanders are independent. **So we will not include gender in our model.**
...
The frequency polygon indicates that there is a clear difference in favor of Bernie Sanders for voters below the age of 40. 
...
Note: We are not going to investigate any interaction with gender since we have concluded that it is not a significant explanatory variable and therefore we don't expect it to have any meaningful interaction with the other explanatory variables.
...
```
None of the 3 interaction models yields a test statistic satisfactory to reject the null hypothesis. **There is no emprical evidence of any interaction between age and race, age and party, and party and race.**
...
$\mathbf {logit(\hat{\pi}_{preference\_sanders}) = -0.1340 -0.0125 age + 0.7135 partyI + 0.5942 partyR + 0.8728 race\_white}$


### Odds Ratio for age

The estimated odds ratio for age is $$\hat{OR}_{age} = exp(c\hat{\beta}_1)$$
...
The estimated odds ratio for partyI is $$\hat{OR}_{partyI} = exp(\hat{\beta}_2)$$
...
The estimated odds ratio for partyR is $$\hat{OR}_{partyR} = exp(\hat{\beta}_3)$$
...
The estimated odds ratio comparing partyI to partyR is $$\hat{OR}_{partyI \space vs \space partyR}=exp(\hat{\beta}_2-\hat{\beta}_3)$$
...
The estimated odds ratio for race_white is $$\hat{OR}_{race_white} = exp(\hat{\beta}_4)$$
